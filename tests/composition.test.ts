...
import { mapEffect, mapSub, mapDispatch, batchSubs, type Effect, type Sub } from "../src/hyperapp";
...
describe("mapSub()", () => {
  it("transforms subscription message type", () => {
    ...
    const inner: Sub<number> = [
      (dispatch, _props) => { dispatch(99); return () => { }; },
      {},
    ];
    const mapped = mapSub(inner, (n: number) => `sub:${n}`);
    const [runner, props] = mapped as [any, any];
  ...
  it("handles batched messages", () => {
    let dispatched: any;
    const inner: Sub<number> = [
      (dispatch) => { dispatch([99, 100]); return () => { }; },
      {},
    ];
    const mapped = mapSub(inner, (n: number) => `sub:${n}`);
    const [runner, props] = mapped as [any, any];
  ...
  it("preserves subscription props", () => {
    let receivedProps: any;
    const inner: Sub<number> = [
      (dispatch, props) => { receivedProps = props; dispatch(1); return () => { }; },
      { interval: 1000 },
    ];
...
describe("mapDispatch()", () => {
  it("transforms dispatched messages", () => {
    const log: string[] = [];
    const parent = (msg: string) => log.push(msg);
...
describe("batchSubs()", () => {
  it("flattens and filters subscriptions", () => {
    const sub1: Sub<string> = [() => () => { }, {}];
    const sub2: Sub<string> = [() => () => { }, {}];
