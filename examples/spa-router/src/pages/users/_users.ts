...
export function userCard(user: UserSummary): VNode {
  return h("a", {
  ...
  h("div", { class: "card-body flex-row items-center gap-4" },
    ...
    h("div", { class: "flex-1 min-w-0" },
      h("h3", { class: "font-semibold" }, user.name),
      h("p", { class: "text-sm text-base-content/60" }, user.role),
    ),
    h("div", { class: "badge badge-ghost" }, `${user.projects} projects`),
  ));
}
